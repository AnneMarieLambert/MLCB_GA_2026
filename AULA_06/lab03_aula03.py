...
from sklearn.tree import DecisionTreeClassifier # Importado conforme LAB 01
...
dados_imobiliaria = [
    ...
    # cancelar_contrato
    ("Quero cancelar o meu contrato de aluguel", "cancelar_contrato"),
    ("Como faço para pedir o distrato do imóvel?", "cancelar_contrato"),
    ("Preciso encerrar o contrato de locação antes do prazo", "cancelar_contrato"),
    ("Gostaria de dar início ao cancelamento do contrato da casa", "cancelar_contrato"),
    ("Quais são as taxas para rescisão contratual e distrato?", "cancelar_contrato")
]
...
RESPOSTAS_PADRAO = {
    ...
    "cancelar_contrato": (
        "**Setor de Rescisões:** Entendemos a sua solicitação. "
        "Para iniciar o processo de distrato ou cancelamento sem burocracia, "
        "envie um e-mail para contratos@imobiliaria.com informando o número do contrato "
        "ou acesse ://imobiliaria.com para analisar as condições."
    )
}
...
def processar_atendimento_sac(mensagem_usuario: str):
    ...
    if confianca >= LIMIAR_CONFIANCA:
        classificacao_status = f" IDENTIFICADO ({intencao_detectada}) [Corte Aplicado: {LIMIAR_CONFIANCA * 100:.0f}%]"
        texto_resposta = RESPOSTAS_PADRAO[intencao_detectada]
    else:
        classificacao_status = f" UNCERTAIN (Fallback Acionado) [Corte Aplicado: {LIMIAR_CONFIANCA * 100:.0f}%]"
        texto_resposta = (
